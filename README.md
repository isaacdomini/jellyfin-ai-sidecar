...
#### 🌐 Floating AI Search for All Web Users (Non-Admins)
The plugin automatically injects a bundled, version-controlled client script (`/Plugins/AiSidecar/ClientScript`) into Jellyfin Web's `index.html` on startup, providing a context-aware **"✨ Ask AI Scene"** floating button for all users:
...
**No manual UI configuration needed**—it activates automatically upon installing the plugin and restarting Jellyfin.
